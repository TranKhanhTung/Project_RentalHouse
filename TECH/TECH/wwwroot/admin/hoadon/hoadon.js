(function ($) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Lưu giá gốc ban đầu
        const baseAmount = parseFloat(tongTienInput.value) || 0;

        // Gán giá gốc vào hidden nếu cần
        tongTienHidden.value = baseAmount;
        ...
        serviceChecks.forEach(cb => {
            cb.addEventListener("change", function () {
                ...
                serviceChecks.forEach(item => {
                    if (item.checked) {
                        extra += parseFloat(item.dataset.price);
                    }
                });

                const newTotal = baseAmount + extra;
                tongTienInput.value = newTotal;
                tongTienHidden.value = newTotal;
            });
        });
    });
